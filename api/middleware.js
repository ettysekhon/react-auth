...
exports.verifyCredentials = (errorIfNoToken) => {
  return (req, res, next) => {
    ...
    // on expiry user will need to login again
    if (token) {
      jwt.verify(token, config.secret, (err, decoded) => {
        ...
        // send error if no user in database and errorIfNoToken
...
exports.errorLogger = (err, req, res, next) => {
  logger.error({ req, res, error: err }, err.stack);
  next(err);
};
